"""Single-ethnicity + mega variant for the salary-age ROC sweep.

Same mega shape as plot_roc_salary_curves_single: aggregate gender-split
and ethnicity-avg rows paired at the top, then 8 per-ethnicity detail
rows below.

Usage:
    python3 plot_roc_salary_age_curves_single.py --group spanish
    python3 plot_roc_salary_age_curves_single.py --all
"""
import argparse
...
from plot_roc_salary_age_curves import plot as plot_roc, plot_by_ethnicity
...
def main():
    ...
    sel.add_argument("--all", action="store_true",
                     help="Render aggregate + per-ethnicity PNGs + mega.")
    ...
    p.add_argument("--mega-out",
                   default=f"results/roc_salary_age/"
                           f"roc_{SAMPLING}_salary_age_all_t{TRIALS}.png",
    ...
    if args.all:
        ...
        eth_avg_path = (
            f"results/roc_salary_age/"
            f"roc_{SAMPLING}_salary_age_ethnicity_avg_t{TRIALS}.png"
        )
        plot_by_ethnicity(df, eth_avg_path)

        written = [aggregate_path, eth_avg_path]
        for group in ETHNICITY_NAMES:
            out = (f"results/roc_salary_age/"
                   f"roc_{SAMPLING}_salary_age_{group}_t{TRIALS}.png")
        ...
        if args.group not in ETHNICITY_NAMES:
            raise SystemExit(
                f"Unknown group {args.group!r}. Known: {list(ETHNICITY_NAMES)}"
            )
        out = args.out or (
            f"results/roc_salary_age/"
            f"roc_{SAMPLING}_salary_age_{args.group}_t{TRIALS}.png"
        )
